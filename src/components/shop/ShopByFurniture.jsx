...
const CustomPrevArrow = ({ onClick }) => (
  <button
    ...
    className="absolute -left-1 md:left-1 lg:left-0 top-[110px] md:top-[180px] xl:top-[215px] 2xl:top-[250px] transform -translate-y-1/2 z-10 cursor-pointer bg-[#FFFFFF]"
  >
    <img
      src={PrevArrowImg}
      alt="Previous"
...
const CustomNextArrow = ({ onClick }) => (
  <button
    ...
    className="absolute -right-1 md:right-1 lg:right-0 top-[110px] md:top-[180px] xl:top-[215px] 2xl:top-[250px] transform -translate-y-1/2 z-10 cursor-pointer bg-[#FFFFFF]"
  >
    <img
      src={NextArrowImg}
      alt="Next"
...
const ShopByFurniture = () => {
  ...
  if (isLoading) {
    return (
      <>
        <div className="w-full max-w-[90%] mx-auto text-center py-10 overflow-x-hidden">
      ...
      <div className="relative">
        <Slider {...settings}>
          {categories?.map((category) => (
            <div key={category.id} className="md:px-2">
              <div className="bg-white p-2 md:p-4">
                <Link to={`/Category/${category.slug}`}>
                  <img
                    src={category.image}
                    alt={category.name}
                    className="w-full h-48 md:h-[300px] lg:h-[380px] xl:h-[450px] 2xl:h-[500px] object-cover aspect-square"
                  />
                  <h3 className="text-lg lg:text-xl xl:text-2xl 2xl:text-3xl font-semibold mt-4 text-[#9C0300]">
                    {category.name}
                  </h3>
                </Link>
              </div>
